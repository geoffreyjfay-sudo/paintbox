// ── Colour science (merged from colour.js) ────────────────────────────────────
...
function blendLab(lab1, lab2, w) {
  return { L: lab1.L * w + lab2.L * (1 - w), a: lab1.a * w + lab2.a * (1 - w), b: lab1.b * w + lab2.b * (1 - w) };
}

function blendRgb(c1, c2, w) {
  return { r: Math.round(c1.r * w + c2.r * (1 - w)), g: Math.round(c1.g * w + c2.g * (1 - w)), b: Math.round(c1.b * w + c2.b * (1 - w)) };
}
...
function weightToRatio(w) {
  const steps = [[1,4],[1,3],[1,2],[2,3],[3,4],[1,1],[3,2],[2,1],[3,1],[4,1]];
...
function suggestMix(targetR, targetG, targetB, paletteColours) {
  ...
  let bestDelta = Infinity, bestResult = null;

  colours.forEach((c, i) => {
    const dE = deltaE(targetLab, labs[i]);
    if (dE < bestDelta) {
      bestDelta = dE;
      bestResult = { parts: [{ colour: c, ratio: 1 }], mixHex: hexFrom(c), deltaE: dE };
    }
  });
  ...
  for (let i = 0; i < colours.length - 1; i++) {
    for (let j = i + 1; j < colours.length; j++) {
      for (const w of weights) {
        const dE = deltaE(targetLab, blendLab(labs[i], labs[j], w));
        if (dE < bestDelta) {
          bestDelta = dE;
          const [ra, rb] = weightToRatio(w);
          bestResult = {
            parts: [{ colour: colours[i], ratio: ra }, { colour: colours[j], ratio: rb }],
            mixHex: hexFrom(blendRgb(colours[i], colours[j], w)),
            deltaE: dE,
          };
        }
      }
...
mixCanvas.addEventListener('click', (e) => {
  ...
  let result;
  try {
    result = suggestMix(r, g, b, activePalette.colours);
  } catch (err) {
    showToast('Mix error — try reloading the page');
    console.error('suggestMix failed:', err);
    return;
  }
